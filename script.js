...
discord-token: ${data.discordToken || ''}


# prefix for old-style text commands, mostly unused
text-prefix: ${data.textPrefix || 'b.'}
...
about:
  ...
  description: >
    ${data.description || 'Collect countryballs on Discord, exchange them and battle with friends!'}

  # override this if you have a fork
  github-link: ${data.githubLink || 'https://github.com/laggron42/BallsDex-DiscordBot'}

  # valid invite for a Discord server
  discord-invite: ${data.discordInvite || 'https://discord.gg/INVITE_CODE'}

  terms-of-service: ${data.termsOfService || 'https://gist.github.com/ # replace with your own link'}
  privacy-policy: ${data.privacyPolicy || 'https://gist.github.com/ # replace with your own link'}


# WORK IN PROGRESS, DOES NOT FULLY WORK
# override the name "countryball" in the bot
collectible-name: ${data.collectibleName || 'countryball'}


# WORK IN PROGRESS, DOES NOT FULLY WORK
# override the name "countryballs" in the bot
plural-collectible-name: ${data.pluralCollectibleName || 'countryballs'}


# WORK IN PROGRESS, DOES NOT FULLY WORK
# override the name "BallsDex" in the bot
bot-name: ${data.botName || 'BallsDex'}


# players group cog command name
# this is /balls by default, but you can change it for /animals or /rocks for example
players-group-cog-name: ${data.playersGroupCogName || 'balls'}


# emoji used to represent a favorited collectible
favorited-collectible-emoji: ${data.favoritedCollectibleEmoji || '❤️'}


# maximum amount of favorites that are allowed
max-favorites: ${data.maxFavorites || 50}


# the highest/lowest possible attack bonus, do not leave empty
# this cannot be smaller than 0, enter a positive number
max-attack-bonus: ${data.maxAttackBonus || 20}


# the highest/lowest possible health bonus, do not leave empty
# this cannot be smaller than 0, enter a positive number
max-health-bonus: ${data.maxHealthBonus || 20}
...
admin-command:
  ...
  guild-ids: 
${data.guildIds.map(id => `    - ${id}`).join("\n")}

  # list of role IDs having full access to /admin
  root-role-ids:
${data.rootRoleIds.map(id => `    - ${id}`).join("\n")}

  # list of role IDs having partial access to /admin
  admin-role-ids:
${data.adminRoleIds.map(id => `    - ${id}`).join("\n")}
  ...
  owners:
    ...
    team-members-are-owners: ${data.teamMembersAreOwners ? 'true' : 'false'}
  ...
  admin-panel:
      ...
      url: ${data.adminPanelUrl || 'http://localhost:8000'}
...
${data.additionalPackages.map(pkg => `    - ballsdex.packages.${pkg.trim()}`).join("\n")}
...
spawn-chance-range: [${data.spawnChanceMin || 40}, ${data.spawnChanceMax || 55}]\n
...
sentry:
  ...
  catch_button_label: "${data.catchButtonLabel || 'Catch me!'}"\n

  # the message that appears when a user catches a ball 
  caught_msgs:
${data.caughtMessages.length === 0 ? `    - "{user} You caught **{ball}**!"\n` : data.caughtMessages.map(msg => `    - "${msg}"`).join("\n")}
...
${data.wrongMessages.length === 0 ? `    - "{user} Wrong name! You put: {wrong}"\n` : data.wrongMessages.map(msg => `    - "${msg}"`).join("\n")}

  # the message that appears above the spawn art
  # {user} is not available here, because who would it ping?
  spawn_msgs:
${data.spawnMessages.length === 0 ? `    - "A wild {collectible} appeared!"\n` : data.spawnMessages.map(msg => `    - "${msg}"`).join("\n")}


  # the message that appears when a user is to slow to catch a ball
  slow_msgs:
${data.slowMessages.length === 0 ? `    - "{user} Sorry, this {collectible} was caught already!"\n` : data.slowMessages.map(msg => `    - "${msg}"`).join("\n")}
`
    return yaml;
}
